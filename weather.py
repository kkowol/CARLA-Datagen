...
class Weather:
    ...
    def set_weather(self, preset):
        if preset=='clear':
            self.sun_angle_alt   = random.randint(20.0, 90.0)
            self.sun_angle_azi   = random.randint(0.0, 360.0)
            self.clouds          = random.randint(abs(self.wetness - 10.0), self.wetness + 10.0)
        
        if preset=='rain':
            self.sun_angle_alt   = random.randint(20.0, 90.0)
            self.sun_angle_azi   = random.randint(0.0, 360.0)
            self.rain            = random.randint(50.0, 100.0)
            self.wetness         = self.rain
            self.puddles         = random.randint(20.0, 50.0)
            self.clouds          = random.randint(40.0, 50.0)
